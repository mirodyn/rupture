...
After the [[Rune Tiles|Rune Tile]] is added to the spell, the player can decide to add new [[Effects|Effect]] to the spell. They can only add such a effect which [[Spell#Incantations|Incantation]] requirement is satisfied by the tile they just added to the spell. If multiple incantations are formed and valid after the tile is added, the player can choose only one of them to use for the acquiring new [[Effects|Effect]].
If the [[Effects|Effect]] is acquired, the player has to decide to either add it to the [[Spell#Spell deck|Spell deck]] or to the [[Effects#Active Effects|Active Effects]]. If player chooses to add the [[Effects|Effect]] the [[Effects#Active Effects|Active Effects]], the [[Effect Cards|Effect Card]] has to be marked by [[Focus Tokens|Focus token]] to clearly mark the connection between the current spell and the effect. Such effects are then called [[Spell#Focused Effects|Focused]]. 
...
Incantations are sequences of [[Runes]] that are formed in the spell. They are used for acquisition of [[Effects]] during the [[Spell#Weaving|Weaving]] of the spell. Each incantation is formed by [[Core Tiles|Core Tile]] with [[Shard Tokens|Shard token]] and one or more runes while the last added [[Rune Tiles|Rune Tile]] is always part of the incantation. Incantation only exists at the moment when new [[Rune Tiles|Rune Tile]] is added to the spell and before the [[Effect Cards|Effect Card]] is selected. It does not exist in meaningful way after that. 
Most of the time, the [[Rune Tiles]] are added to the spell, multiple Incantations are created, but only one of them can be used each turn and most of them will usually be useless as no [[Effects]] will require them. Each Incantation has to be "read" in the direction starting from the [[Core Tiles|Core Tile]], leading towards the last added tile, it has to contain the last added tile and cannot contain any tile multiple times.
...
By adding the tile **X** following valid incantations were created:
...
>[!WARNING] Invalid Incantations
> D -> E -> B : does not contain the last added tile
> X -> A : goes "against" the direction of the incantation.  

>[!TODO] TODO: Inconsistent ruling on "self crossing" Incantations
> What about X -> D -> A ??? Its valid in terms of direction and containing the last added tile, but it feels wrong to use tile that "had to be crossed" on the way from the spell core to the "start" of the incantation itself...
# Spell deck

This is a deck of [[Effect Cards]] placed in [[Game Layout#Player Area|Player Area]]. It serves as a "payload" of the spell and is used during the [[Spell#Release|Release]]. Cards in this deck are face down and are not public. Only the owner of the spell can take a look at the card in this deck. Every time, new [[Effect Cards]] are acquired, the player can choose to put them into Spell Deck. [[Effects]] in spell deck has no influence on the game, before the [[Spell#Release|Release]] of the spell.  
# Focused Effects

These are [[Effect Cards]] placed directly into the [[Effects#Active Effects|Active Effects]] by the player. This typically happens during the [[Spell#Weaving|Weaving]] of the spell. All of these [[Effect Cards]] has to be marked by [[Focus Tokens]] that tells the player, that this specific Effect is directly tied to the spell itself. These effects can be lost trough [[Ripple]] event, any other source of [[Spell#Collapse|Collapse]] or when the spell is [[Spell#Release|Released]].
# Release
...
- [[Effects#Status Deck|Status Deck]] of ANY player has to be chosen as a target of the spell.
- All the cards from the [[Spell#Spell deck|Spell deck]] are shuffled into the target [[Effects#Status Deck|Status Deck]].
...
- If the [[Initiative - Balance token]] is not turned to its Balance side yet, turn it to that side. 
# Collapse

If for any reason any number of [[Rune Tiles]] becomes disconnected (Most common reason will be the [[Ripple]] event.) form the rest of the spell, the collapse of that tile(s) is triggered and needs to get resolved immediately. To resolve the collapse the player has to return the affected [[Rune Tiles]] to the [[Rune Tile Bag]] and has to discard on of his [[Effect Cards]] following these rules:
- Find all the [[Effect Cards]] between your [[Spell#Focused Effects|Focused Effects]] and [[Spell#Spell deck|Spell deck]] with their [[Spell#Incantations|Incantation]] requirement ENDS with the same [[Runes|Rune]] as the collapsed tile provides. Choose one of them and discard it. 
- If no [[Effect Cards]] was found during the first step, try to find all [[Effect Cards]] between your [[Spell#Focused Effects|Focused Effects]] and [[Spell#Spell deck|Spell deck]] with their [[Spell#Incantations|Incantation]] requirement CONTAINS with the same [[Runes|Rune]] as the collapsed tile provides.
